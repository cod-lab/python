...
def gameloop():
    ...
    while exitgame != True:
        if gameover == True:
            w.PlaySound(r"death.wav", w.SND_ASYNC | w.SND_ALIAS )                #gives sound effects in windows
            #ps('death.wav')
            #ps('',False)
            #ps('end.wav')
            w.PlaySound(r"end.wav", w.SND_ASYNC | w.SND_ALIAS )                #gives sound effects in windows
            #song = aus.from_wav("The Rowdy Tavern.wav")
            #p(song)
            boxwindow.fill(grey)
            text_screen("GAME OVER!","press ENTER to play again...",blue,yellow,250,160,230,200)
            #w.PlaySound("The Rowdy Tavern.wav", w.SND_ALIAS )                #gives sound effects in windows
            ...
            if abs(snake_x - food_x)<10 and abs(snake_y - food_y)<10:
                ...
                w.PlaySound("f.wav", w.SND_ASYNC | w.SND_ALIAS )                #gives sound effects in windows
                score += 10
                snake_length += 2
                food_x = r.randint(0,box_width)
                food_y = r.randint(0,box_height)
