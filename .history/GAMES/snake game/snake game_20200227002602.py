            ...
            for event in pg.event.get():
                ...
                if event.type == pg.KEYDOWN:
                    if event.key == pg.K_RETURN:
                        #w.PlaySound(None, w.SND_ALIAS)                      #stops currently playing sound('end.wav') by playing 'none'
                        w.PlaySound('end.wave', w.SND_PURGE)                     #Stop playing all instances of the specified sound
                        gameloop()
        else:
            for event in pg.event.get():
                if event.type == pg.QUIT:
