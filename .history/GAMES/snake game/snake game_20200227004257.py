...
import random as r
import tkinter as t
import winsound as w                                                        #use to add sound(for windows only)
    ...
    while exitgame != True:
        if gameover == True:
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.KEYDOWN:
                    if event.key == pg.K_RETURN:
                        #w.PlaySound(None, w.SND_ALIAS)              #plays 'none', stops currently playing sound('end.wav') by playing 'none'
                        w.PlaySound(None, w.SND_PURGE)                      #Stop playing all instances of the specified sound
                        gameloop()
        else:
            for event in pg.event.get():
                if event.type == pg.QUIT:
            ...
            if abs(snake_x - food_x)<10 and abs(snake_y - food_y)<10:
                w.PlaySound("f.wav", w.SND_ASYNC)                           #plays sound whenever snake eats food
                #ps('f.wav')
            ...
            if (snake_head in snake_list[:-1]) or (snake_x or snake_y)<0 or (snake_x>box_width) or (snake_y>box_height):
                #list[-1]or[:-1] -> starting from last item
                gameover = True
                w.PlaySound("round_end.wav", w.SND_NOSTOP)                  #plays sound when snake gets killed himself
                w.PlaySound("end.wav", w.SND_ASYNC)                         #plays sound when game is over
