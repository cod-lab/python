...
class person():
    ...
    def fn2(self):
        ...
        if z!=self.country:
            ...
            self.update_json('outsider.json')
    ...
    def update_json(self,file):
        ...
        temp={}
        with open(file,'r') as f:                   #load outsider.json file into ram
            temp=json.load(f)                       #print outsider.json file data here

        self.n+=len(temp)                           #count no. of keys in dict and add into n
        ...
        with open(file,'w') as f:                   #creates json file on hdd(same folder) if not exist already and open it internally
            print(json.dump(temp,f))                #add data into outsider.json file
