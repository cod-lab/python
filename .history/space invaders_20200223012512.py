...
#collision fn for multiple collisions using 'pythagoras theoram'
def collision(t1,t2):
    distance = math.sqrt(math.pow(t1.xcor()-t2.xcor(),2) + math.pow(t1.ycor()-t2.ycor(),2))     # math.pow(a,b), a-base,b-power
    # if abs(t1.xcor() - t2.xcor())<10 and abs(t2.ycor() - t2.ycor() not working
    if math.sqrt(math.pow(t1.xcor()-t2.xcor(),2) + math.pow(t1.ycor()-t2.ycor(),2))<10:                                                                             # math.sqrt(c)
        return True
    else:
        return False
