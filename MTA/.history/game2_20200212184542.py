...
boxwindow = pg.gamewindow.set_mode((box_width,box_height)) #display.set_mode((box_width,box_height))
...
while not exitgame:
    ...
    #collision
    c = 10
    if abs(snake_x - food_x)<c and abs(snake_y - food_y)<c:
        score += 10
        snake_length += 5
        food_x = r.randint(0,box_width)
        food_y = r.randint(0,box_height)
