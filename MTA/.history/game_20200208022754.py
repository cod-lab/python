...
food_x = r.randint(0,screen_width1)
food_y = r.randint(0,screen_height1)

gamewindow = pg.display.set_mode((screen_width,screen_height))
gamewindow1 = pg.display.set_mode((screen_width1,screen_height1))
...
def plot_snake(gamewindow,black,snk_list,snake_size2):
    for x,y in snk_list:
        pg.draw.rect(gamewindow1,black,[x,y,snake_size2,snake_size2])
...
while not exitgame:
    ...
    #collision
    if abs(food_x - snake_x)<score and abs(food_y - snake_y)<score:
        score += 10
        snake_size += 10
        print("score=",score)
